...
const buildSizePrefixRe = new RegExp(
  `^${escape_string_regexp_1.default(buildSizePrefix)}(.+)$`,
  'm',
);
/**
...
function outputChanges(changes) {
  ...
  for (const [oldFile, newFile] of changes.changedItems.entries()) {
    ...
    if (oldFile.gzipSize === newFile.gzipSize) {
      ...
      const sizeDiff = pretty_bytes_1.default(newFile.gzipSize - oldFile.gzipSize, {
        signed: true,
      });
      const relativeDiff = Math.round((newFile.gzipSize / oldFile.gzipSize) * 1000) / 1000;
      size =
        `${pretty_bytes_1.default(oldFile.gzipSize)} -> ${pretty_bytes_1.default(
          newFile.gzipSize,
        )}` +
        ' (' +
